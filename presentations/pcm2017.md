...
- File or folder names contain `test`, `beta`, `v[0-9]{1}`
...
Git Frontend (GitLab, BitBucket, etc) has to be configured so that merge into development branch is not possible without accepting a **pull request** (= code review).
