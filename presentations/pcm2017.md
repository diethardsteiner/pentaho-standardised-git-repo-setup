...
- **10 years of PCM!** How amazing is this?!
- **Upfront warning**: This will be a pretty **boring talk**!
...
Feature branches get merged into dev:

![](pics/git-merge-squash.png)

If `f1` branch gets merged, a tag `1.0.4-dev` gets created and later `f2` later gets merged: If we checkout `1.0.4-dev` we will also get any commits coming from `f2` at this stage since the whole history got merged. So if we were to put this code into production, we would have partially finished `f2` features in it, which is - of course - a problem. Do we need the history of features? Not really, not each commit. The solution is to squash all the commits for all merges, which basically compresses the history into just one commit (so all the history gets preserved, it’s like one single **changelog** instead of many individual ones). The command to use is:

```
merge squash ...
```

Over the long run feature branches get deleted.
