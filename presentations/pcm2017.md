...
| `pentaho-server/metadata-models`   | pentaho metadata models
| `pentaho-server/mondrian-schemas`  | mondrian cube definitions
| `pentaho-server/content`           | contains export from pentaho server repo 
| `sql`                              | sql queries
| `sql/ddl`                          | ddl
| `prd`                              | perntaho report files. Alternatively could be stored in `pentaho-server/content`, but then it is not export only???
...
| folder                             | description
|------------------------------------|---------------------------------
| `.kettle`                          | pdi config files
| `shell-scripts`                    | shell scripts to execute e.g. PDI jobs
| `test-data`                        | optional: test data for development or unit testing - specific to environment 
